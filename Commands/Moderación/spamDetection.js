	...
	async execute(interaction, client) {
		...
		try {
			...
			try {
								...
								const embed4 = new EmbedBuilder()
									...
									.addFields(
										...
										},
										{
											...
											value: `**${timeoutSecons ? `${timeoutM} minutos` : `60 segundos`
								...
								const embed5 = new EmbedBuilder()
									...
									.addFields(
										...
										},
										{
											...
											value: `**${permChannel ? `${permChannel}` : `Ningun canal`
												}** esta habilitado para que cualquier miembro envie links.`,
										},
										{
											name: `Rol permitodo:`,
											value: `**${permRol ? `${permRol}` : `Ningun Rol`
							...
							if (automodRules.length === 0) {
								...
								automodRules.forEach((rule) => {
									...
									const ruleInfo = [
										...
										`**Estado:** ${rule.enabled ? "Habilitado" : "Deshabilitado"
										}`,
									].join("\n");
				...
				await interaction.channel.send("No tengo permisos para lanzar mi sistema de AutoMod en este servidor :c");
			}
		} catch (error) {
